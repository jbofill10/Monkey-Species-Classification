...
def visualize():
    ...
    xception_2_epoch_df = pd.read_pickle('Data/pickles/xception_2_epoch_df')

    xception_2_epoch_df['epoch'] = list(map(lambda x: x + 1, list(xception_2_epoch_df.index)))

    fig = go.Figure(data=[
        go.Scatter(x=xception_2_epoch_df['epoch'], y=xception_2_epoch_df['accuracy'], line={'dash': 'solid'}, name='Train'),
        go.Scatter(x=xception_2_epoch_df['epoch'], y=xception_2_epoch_df['val_accuracy'], line={'dash': 'solid'},
                   name='Validation')
    ])
    ...
    fig.update_layout(
        xaxis_title='Epochs',
        yaxis_title='Accuracy',
        title='Train Accuracy vs. Validation Accuracy - Xception with Flatten',
        width=1200,
        height=800
    )
    ...
    fig = go.Figure(data=[
        go.Scatter(x=xception_2_epoch_df['epoch'], y=xception_2_epoch_df['loss'], line={'dash': 'solid'}, name='Train'),
        go.Scatter(x=xception_2_epoch_df['epoch'], y=xception_2_epoch_df['val_loss'], line={'dash': 'solid'},
                   name='Validation')
    ])
    ...
    fig.update_layout(
        xaxis_title='Epochs',
        yaxis_title='Loss',
        title='Train Loss vs. Validation Loss - Xception with Flatten',
        width=1200,
        height=800
    )
